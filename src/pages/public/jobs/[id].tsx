...
export default function PublicJobView() {
  ...
  const fetchJobDetails = async () => {
    try {
      const { data: jobData, error: jobError } = await supabase.from("jobs").select("*").eq("job_id", id).single()

      if (jobError) throw jobError
      setJob(jobData)

      // Fetch tasks
      const { data: taskData, error: taskError } = await supabase.from("tasks").select("*").eq("job_id", id)
